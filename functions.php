...
<?php
/**
 * CafeVzla functions and definitions
 */
...
function thebox_scripts() {
	...
	wp_enqueue_style( 'thebox-style', get_stylesheet_uri(), array(), '1.4.17' );
...
function thebox_excerpt($num) {
	...
	array_pop($excerpt);
	$excerpt = implode(" ",$excerpt)."... <br><a class=\"more-link\" href='" .get_permalink($post->ID) ." '>".__('Leer más', 'thebox')." &raquo;</a>";
	echo $excerpt;
}
...
function thebox_credits() {
	$website_credits = '';
	$website_author = get_bloginfo('name').', Elaborado por la Coordinación de Tecnología de la Información RCO';
	$website_date =  date ('Y');
